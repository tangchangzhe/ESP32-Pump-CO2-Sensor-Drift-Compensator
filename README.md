...
1. **NTP时间同步**：每个采集周期开始前，ESP32主动与NTP服务器同步时间，纠正内部RTC时钟的累积漂移（嵌入式设备的晶振误差可能导致每天数秒至数十秒的偏差）
2. **整点对齐**：所有数据时间戳对齐到10分钟整点（`00:00:00`, `00:10:00`, `00:20:00`...），保证数据间隔均匀

**时间戳对齐的意义**：

| 潜在问题 | 未对齐时的影响 | 对齐机制的作用 |
| -------- | -------------- | -------------- |
| RTC累积漂移 | 运行一周后时间戳可能偏差数分钟，数据与真实事件（如日出日落）产生错位 | 每次采集前NTP校准，确保长期运行的时间精度 |
| 采样间隔不均匀 | 数据点间隔波动（如9分50秒、10分15秒），影响变化率计算的准确性 | 严格等间隔采样，保证数学分析与插值的可靠性 |
...
**实现细节**：
- 单次测量耗时约45秒（气泵采样25秒 + 气压稳定20秒），但上传的时间戳是**计划采集时间**而非完成时间
- 每次测量完成后，系统与NTP服务器校准时间，并计算下一个10分钟整点作为下次采集时间；断电恢复后同理
...
1. **锚点选择（周期闭环法）**：选择完整光暗周期的起止点作为锚点。理论上经过一个完整周期后，CO2 应回归相近水平；实测差值即为该周期内的累积漂移。本质上，漏气使原本的周期性波动叠加了一个下降趋势，补偿算法的作用就是将倾斜的基线抬平，还原真实的生物活动信号
   >  注意：周期闭环法误差较大，仅适用于漂移趋势明显可辨的场景，用于还原大致的周期性波动图像，不建议用于精密定量分析
